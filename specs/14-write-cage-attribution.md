...
**Amendment 2026-07-29:** an EMPTY .write sidecar in queue/ parks instantly (marker reason token write-target-empty, speedwars class write-target-missing, retryable=0) — the bounded wait exists for targets that may yet be created; an empty target never can be. write-target-empty joins FR-7's marker-only token list.
